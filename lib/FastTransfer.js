...
if (RUNTIME !== 'browser') {
    ...
    exports.functions.getLocalIP = getLocalIP;
    ...
    class Arguments {
        constructor(argv, options = {}) {
            ...
            for (const o in this.options) {
                ...
                for (const a in this.options[o].aliases) {
                    const aname = this.options[o].aliases[a].startsWith('--') ? this.options[o].aliases[a].substring(2) : this.options[o].aliases[a].substring(1);
                    this.options[aname] = { ...this.options[o] }; // clone
                    this.options[aname].name = o;
                    delete this.options[aname].default;
                    delete this.options[aname].aliases;
                }
            }
        }
        _parseValue(arg, type = null) {
            if (!type || type === 'S') return arg;
            if (type === 'B') return (arg.toLowerCase() === 'true' || arg.toLowerCase() === '1');
            if (type === 'N') {
                if (isNaN(arg)) throw new Error('Invalid number: ' + arg);
                return Number(arg);
            }
        ...
        parse() {
            ...
            const args = { _: [] };
            for (const o in this.options) {
                if (typeof this.options[o].default !== 'undefined') args[o] = this.options[o].default;
            }
            ...
            while (argv.length > 0) {
                ...
                if (arg.startsWith('-')) {
                    const aname = arg.startsWith('--') ? arg.substring(2) : arg.substring(1);
                    if (!this.options[aname]) {
                        logger.error('MAIN', 'Unknown argument: ' + arg);
                        logger.error('MAIN', 'See --help for all valid arguments.');
                        process.exit(1);
                    }
                    if (lastArg !== null) {
                        logger.error('MAIN', 'Invalid value for argument "--' + lastArg + '": No value provided.');
                        logger.error('MAIN', 'See --help for all valid arguments.');
                        process.exit(1);
                    }
                    if (this.options[aname].type !== 'P') {
                        lastArg = aname;
                    } else {
                        args[this.options[aname].name] = true;
                    }
                } else if (lastArg !== null) {
                    try {
                        args[this.options[lastArg].name] = this._parseValue(arg, this.options[lastArg].type);
                    } catch (e) {
                        logger.error('MAIN', 'Invalid value for argument "--' + lastArg + '": ' + e.message);
                        logger.error('MAIN', 'See --help for all valid arguments.');
                        process.exit(1);
                    }
                    lastArg = null;
    ...
    async function mainCLI() {
        ...
        if (args.help || (args._.length > 0 && args._[0].toLowerCase() === 'help')) {
            ...
            console.log(`  ${args.binary} ${args.script} share [files-to-share] [options]`);
            console.log('Commands:');
            console.log('  help                             Show this help message.');
            console.log('    --help, -h');
            //console.log('  serve                            Start a server waiting to receive files. (default)');
            console.log('  share [files-to-share]           Start a server sharing files.');
            ...
            console.log('Options:');
            console.log('  --log-level N                    Set the logging level (0-4). (default: 3)');
            console.log('  --local, -l                      Do not use cloudflared to expose the server to the internet. (default: false)');
            //console.log('  --no-prompt                      Do not ask for confirmation before accepting incoming files in serve mode.');
            console.log('  --threads, -t N                  Number of threads (concurrent connections) to use when downloading files. (default: 4)');
            process.exit(0);
        }
        ...
        if (args._[0] == 'share') {
            ...
            if (!args.local) {
                ...
                code = `r:${server.key}:http:${await getLocalIP()}:${address.port}`;
                logger.log('MAIN', 'Local trasfer code (local address may not be accurate):', code);
            }
            ...
            let logQueue = [];
            setImmediate(async () => {
                let realBytesDownloaded = (bytesDownloaded - bytesAlreadyDownloaded);
                let lastRealBytesDownloaded = 0;
                while (filesDownloaded < info.files.length || logQueue.length > 0) {
                    if (logQueue.length > 0) {
                        lineWriter.clear();
                        while (logQueue.length > 0) {
                            const log = logQueue.shift();
                            switch (log[0]) {
                                case 'log': logger.log(log[1], log[2]); break;
                                case 'warn': logger.warn(log[1], log[2]); break;
                                case 'error': logger.error(log[1], log[2]); break;
                                case 'info': logger.info(log[1], log[2]); break;
                            }
                        }
                    }
                    lineWriter.rewrite(`Downloaded ${filesDownloaded}/${info.files.length} files - ${formatBytes(bytesDownloaded)}/${formatBytes(totalSize)} (${formatBytes(realBytesDownloaded - lastRealBytesDownloaded)}/s - ${formatBytes(realBytesDownloaded / ((Date.now() - startTime) / 1000))}/s avg)`);
                    lastRealBytesDownloaded = realBytesDownloaded;
                    await new Promise(resolve => setTimeout(resolve, 500));
                    realBytesDownloaded = (bytesDownloaded - bytesAlreadyDownloaded);
                }
                lineWriter.clear();
                logger.log('MAIN', `Download (${formatBytes(totalSize)}) complete in ${Math.round((Date.now() - startTime) / 1000)}s (${formatBytes(realBytesDownloaded / ((Date.now() - startTime) / 1000))}/s)`);
                process.exit(0);
            });
            for (const file of info.files) {
                ...
                const tmppath = path + '.fttmp';
                let startIndex = 1;
                let startBytes = 0;
                if (fs.existsSync(path)) {
                    if (await fs.promises.stat(path).then(s => s.size) == file.size) {
                        logQueue.push(['info', 'MAIN', `File "${path}" is already downloaded. Skipping...`]);
                        ...
                        logQueue.push(['warn', 'MAIN', `File "${path}" is the wrong size. Restarting transfer...`]);
                        await fs.promises.unlink(path); // need better handling here
                    }
                }
                if (fs.existsSync(tmppath)) {
                    const stat = await fs.promises.stat(tmppath);
                    if (stat.size > file.size) {
                        logQueue.push(['warn', 'MAIN', `Previous download of "${path}" is larger than expected. Restarting transfer...`]);
                        await fs.promises.writeFile(tmppath, Buffer.alloc(0));
                    } else {
                        startIndex = Math.floor(stat.size / DEFAULT_PART_SIZE) + 1;
                        startBytes = (startIndex - 1) * DEFAULT_PART_SIZE;
                        logQueue.push(['info', 'MAIN', `Attempting to resume download of "${path}" from part ${startIndex}...`]);
                        bytesDownloaded += startBytes;
                        bytesAlreadyDownloaded += startBytes;
                    }
                } else {
                    await fs.promises.writeFile(tmppath, Buffer.alloc(0));
                }
                const fd = await fs.promises.open(tmppath, 'r+');
                let writeIndex = parseInt(startIndex);
                let start = Date.now();
                let running = 0;
                for (let i = startIndex; i <= file.parts; i++) {
                    running++;
                    const ii = parseInt(i);
                    setImmediate(async () => {
                        const data = await client.getPart(file.path, ii);
                        while (writeIndex != ii) await new Promise(resolve => setTimeout(resolve, 10));
                        await fd.write(data, 0, data.length, (writeIndex - 1) * DEFAULT_PART_SIZE);
                ...
                logQueue.push(['log', 'MAIN', `Downloaded "${path}" (${formatBytes(file.size)}) in ${Math.round((Date.now() - start) / 1000)}s (${formatBytes((file.size - startBytes) / ((Date.now() - start) / 1000))}/s)`]);
                filesDownloaded++;
            }
        } else {
            logger.error('ERROR', 'No valid command or transfer code provided.');
